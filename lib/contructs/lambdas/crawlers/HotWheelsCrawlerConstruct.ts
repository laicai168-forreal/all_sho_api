...
export interface HotWheelsCrawlerConstructProps {
    bucket: IBucket;
    secret: ISecret;
    vpc: IVpc;
...
export class HotWheelsCrawlerConstruct extends Construct {
    public readonly function: lambda.Function;

    constructor(scope: Construct, id: string, props: HotWheelsCrawlerConstructProps) {
        super(scope, id);

        const { bucket, secret, vpc, carRDSInstance, logsTable, layer } = props;

        const lambdaSG = new ec2.SecurityGroup(this, "HotWheelsCrawlerLambdaSG", {
            vpc,
            allowAllOutbound: true,
        });

        this.function = new lambda.Function(this, 'CrawlerHotWheelsLambda', {
            runtime: lambda.Runtime.PYTHON_3_12,
            code: lambda.Code.fromAsset('lambda/crawler'),
            handler: 'hot_wheels_crawler.handler',
            environment: {
