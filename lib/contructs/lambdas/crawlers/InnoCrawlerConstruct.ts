...
export interface InnoCrawlerConstructProps {
    bucket: IBucket;
    secret: ISecret;
    vpc: IVpc;
...
export class InnoCrawlerConstruct extends Construct {
    public readonly function: lambda.Function;

    constructor(scope: Construct, id: string, props: InnoCrawlerConstructProps) {
        super(scope, id);

        const { bucket, secret, vpc, carRDSInstance, logsTable } = props;

        const lambdaSG = new ec2.SecurityGroup(this, "InnoCrawlerLambdaSG", {
            vpc,
            allowAllOutbound: true,
        });

        const crawlerDeps = new lambda.LayerVersion(this, 'InnoCrawlerDepsLayer', {
        ...
        this.function = new lambda.Function(this, 'CrawlerInnoLambda', {
            runtime: lambda.Runtime.PYTHON_3_12,
            code: lambda.Code.fromAsset('lambda/crawler'),
            handler: 'inno_crawler.handler',
            environment: {
