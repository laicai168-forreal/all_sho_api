...
export interface TWCrawlerConstructProps {
    bucket: IBucket;
    secret: ISecret;
    vpc: IVpc;
...
export class TWCrawlerConstruct extends Construct {
    public readonly function: lambda.Function;

    constructor(scope: Construct, id: string, props: TWCrawlerConstructProps) {
        super(scope, id);

        const { bucket, secret, vpc, carRDSInstance, logsTable } = props;

        const lambdaSG = new ec2.SecurityGroup(this, "TWCrawlerLambdaSG", {
            vpc,
            allowAllOutbound: true,
        });

        const crawlerDeps = new lambda.LayerVersion(this, 'TWCrawlerDepsLayer', {
        ...
        this.function = new lambda.Function(this, 'CrawlerTWLambda', {
            runtime: lambda.Runtime.PYTHON_3_12,
            code: lambda.Code.fromAsset('lambda/crawler'),
            handler: 'tarmac_works_crawler.handler',
            environment: {
