...
def crawl_inno_product_page(url, historical_image_urls, log, s3_bucket=S3_BUCKET):
    ...
    title = soup.select_one('.product_title').get_text()
    rows = soup.select('.jet-listing-dynamic-field__content')
    ...
    for r in rows:
        r_text = r.get_text()
        if r_text and "SKU" in r_text:
            sku = r_text.replace("SKU", '', 1)
        elif r_text and "Scale" in r_text:
            if "1/64" in r_text:
                scale = "1:64"
            elif "1/18" in r_text:
                scale = "1:18"
        elif r_text and "Type" in r_text:
            cast = r_text.replace("Type", '', 1)

    make = soup.select_one(".jet-listing-dynamic-terms__link").get_text()

    imgs = soup.select('.iconic-woothumbs-images__image')
    img_urls = list(set([i.get('data-large_image') for i in imgs]))
    sorted_urls = sorted(img_urls, key=natural_key)
    image_s3_key = None
    s3_image_urls = []

    for iu in sorted_urls:
    ...
    log(f"### Crawled: {sku}, {make}, {title}, {scale}, {url}")
    ...
    try:        
        ...
        if not override: 
            ...
            for hu in historical_urls:
                log(
                    f"### Crawled: {hu}, this url is skipped because override mode is OFF"
                )
        ...
        for u in urls:
            try:
                item = crawl_inno_product_page(u, historical_image_urls, log, s3_bucket=S3_BUCKET)
