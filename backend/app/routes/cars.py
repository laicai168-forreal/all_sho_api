...
def update_admin_car(request: Request, car_id: str, body: CarMutationRequest):
    sub = get_current_user_sub(request)
    return car_service.update_admin_car(sub, car_id, body.dict(exclude_none=True))


@router.delete("/admin/cars/{car_id}")
def delete_admin_car(request: Request, car_id: str):
    sub = get_current_user_sub(request)
    return car_service.delete_admin_car(sub, car_id)


@router.post("/admin/cars/{car_id}/duplicate")
def duplicate_admin_car(request: Request, car_id: str, body: CarDuplicateRequest):
    sub = get_current_user_sub(request)
    return car_service.duplicate_admin_car(sub, car_id, body.dict(exclude_none=True))
...
def review_admin_change_request(request: Request, request_id: str, body: CarChangeRequestReview):
    sub = get_current_user_sub(request)
